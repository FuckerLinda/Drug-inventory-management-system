...
namespace 药品进销存管理系统
{
    public partial class customer : Form
    {
        ...
        private Boolean Check(String id)
        {
            SqlConnection con = DBConnect.con();
            con.Open();
            String sql = "select * from customer where uid='" + id + "'";
            ...
            if (id.Text != "")
            {
                if (ifadmin == 1)
                {
                    if (!Check(id.Text))
                    {
                        ...
                        cmd.CommandText = "insert into customer values('" + id.Text + "','" + name.Text + "','" + sex + "','" + telephone.Text + "')";
                        cmd.Connection = con;
                        cmd.ExecuteNonQuery();
                        String sql = "select * from customer";
                        SqlDataAdapter Adpt = new SqlDataAdapter(sql, con);
                    ...
                    if (id.Text != "")
                    {
                        if (Check(id.Text))
                        {
                            try
                            {
                                ...
                                cmd.CommandText = "update customer set name='" + name.Text + "',sex='" +sex + "',telephone='" + telephone.Text + "' where uid='" + id.Text + "'";
                                cmd.Connection = con;
                                cmd.ExecuteNonQuery();
                    ...
                    if (id.Text != "")
                    {
                        if (Check(id.Text))
                        {
                            try
                            {
                                ...
                                con.Open();
                                SqlCommand cmd = new SqlCommand();
                                cmd.CommandText = "delete from customer where uid='" + id.Text + "'";
                                cmd.Connection = con;
                                cmd.ExecuteNonQuery();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                // SqlCommand cmd = new SqlCommand();
                // cmd.CommandText = "select * from customer where uid like '%" + id.Text + "%' and name like '%" + name.Text + "%' and count like '%" + count.Text + "%' and telephone like '%" + telephone.Text+"%'" ;
                // cmd.Connection = con;
                // cmd.ExecuteNonQuery();
                // String sql = "select * from customer";
                if (count.Text == "女") sex = 1;
                ...
                String sql;
                if (sex==0 || sex==1)
                sql = "select * from customer where uid like '%" + id.Text + "%' and name like '%" + name.Text + "%' and sex like '%" + sex + "%' and telephone like '%" + telephone.Text + "%'";
                else sql= "select * from customer where uid like '%" + id.Text + "%' and name like '%" + name.Text + "%' and telephone like '%" + telephone.Text + "%'";
                SqlDataAdapter Adpt = new SqlDataAdapter(sql, con);
                DataSet ds = new DataSet();
                Adpt.Fill(ds, "customer");
                dataGridView1.DataSource = ds.Tables[0].DefaultView;
